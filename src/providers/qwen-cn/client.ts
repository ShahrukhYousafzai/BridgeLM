 ...
 */
export async function chatQwenCn(
  ...
  if (!res.ok) {
    ...
    throw new Error(`Qwen China API error ${res.status}: ${text}`);
  }
...
/**
 * Send a streaming chat request to Qwen China (千问).
 * Parses the SSE response and calls the callback for each chunk.
 */
export async function chatStreamQwenCn(
  credentials: ProviderCredentials,
  params: ChatParams,
