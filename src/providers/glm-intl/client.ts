...
const API_URL = 'https://chat.z.ai/api/chat/completions';

/**
 * Build request headers for GLM International API calls.
 */
...
/**
 * Send a non-streaming chat request to GLM International.
 * Collects all SSE chunks and returns an aggregated response.
 */
export async function chatGlmIntl(
  ...
  if (!res.ok) {
    ...
    throw new Error(`GLM International API error ${res.status}: ${text}`);
  }
...
/**
 * Send a streaming chat request to GLM International.
 * Parses the SSE response and calls the callback for each chunk.
 */
export async function chatStreamGlmIntl(
  credentials: ProviderCredentials,
  params: ChatParams,
